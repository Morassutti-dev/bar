...
def listar_comandas(request):
    ...
    # Filtra as comandas com base na busca
    query = request.GET.get('q')
    if query:
        comandas = comandas.filter(
            Q(cliente__icontains=query) |  # Busca pelo nome do cliente
            Q(status__icontains=query)     # Busca pelo status
        )

    # Calcula o total de cada comanda
    comandas_com_total = []
    for comanda in comandas:
        total = comanda.calcular_total()
        comandas_com_total.append({
            'comanda': comanda,
            'total': total
        })

    # Paginação
    paginator = Paginator(comandas_com_total, 10)  # 10 comandas por página
    page_number = request.GET.get('page')
    page_obj = paginator.get_page(page_number)

    # Passa as comandas paginadas e a query de busca para o template
    return render(request, "listar_comandas.html", {
        "page_obj": page_obj,
        "query": query  # Passa a query para manter o valor no campo de busca
    })
...
def editar_cliente(request, comanda_id):
    comanda = get_object_or_404(Comanda, id=comanda_id)
    ...
    if request.method == "POST":
        ...
        comanda.save()
        messages.success(request, "Nome do cliente atualizado com sucesso!")
